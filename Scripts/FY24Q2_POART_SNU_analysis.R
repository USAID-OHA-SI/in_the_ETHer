...
#file_path <- return_latest(folderpath = merdata, pattern = "OU_IM_FY22")
file_path <- return_latest(folderpath = merdata, pattern = "PSNU_IM_FY22-24.*Ethiopia")
...
#grabr::pano_extract_msd(operatingunit = "Ethiopia", 
...
#Updated woreda list
  #note: doesn't include datim_uid's so unable to match on this with MSD 
  #note: woreda list broken into 2 sheets - 1st for USAID and 2nd for CDC 

new_usaid_woreda <- data_folder %>% 
  return_latest("SNUs_ USAID vs Non USAID SNUs as of FY24_SAPR, June 10, 20.xlsx") %>% 
  read_xlsx(sheet = "USAID SNUs") %>% 
  rename(cop22psnu = Woreda,
         psnu = Zone,
         snu1 = Region,
         usaid_supported = ...5) %>% 
  mutate(usaid_supported = "USAID Supported")

#new_cdc_woreda <- data_folder %>% 
 # return_latest("SNUs_ USAID vs Non USAID SNUs as of FY24_SAPR, June 10, 20.xlsx") %>% 
  #read_xlsx(sheet = "Non USAID (CDC Only) SNUs") %>% 
  #rename(cop22psnu = Woreda,
   #      psnu = Zone,
    #     snu1 = Region)
...
df_sites <- df %>% 
  ...
  left_join(new_usaid_woreda %>%
              select(cop22psnu, usaid_supported),
            by = c("cop22_psnu" = "cop22psnu")) %>% #match on cop22psnu since we don't have uid
...
df_site_viz %>% 
  ...
  labs(x = NULL, y = NULL, 
       ...
       subtitle ="# USAID-Supported PSNUs/ # Total PSNUs",
       #subtitle = glue("<span style = 'color:{denim}'># USAID-Supported PSNUs</span>/<span style = 'color:{scooter_light}'> # Total PSNUs</span>"),
       caption = glue("{metadata$caption}| Ref ID: {ref_id}"))# +
  #annotate(geom = "curve", x = 16.5, y = 35, xend = 15.25, yend = 45, 
   # curvature = .5, arrow = arrow(length = unit(2, "mm")))
  

si_save("Graphics/00_psnu_dist.svg")
si_save("Images/00_psnu_dist.png")
#si_save("Images/00_psnu_dist_arrow.png")
...
df_viz <-  df %>% 
  ...
  filter(funding_agency == "CDC",
         ...
         fiscal_year == metadata$curr_fy, #pulls current FY
         psnuuid %in% clinical_cdc_psnu,
  ...
  reshape_msd() %>% 
  left_join(new_usaid_woreda %>%
              select(cop22psnu, usaid_supported), #sub woreda for uid
            by = c("cop22_psnu" = "cop22psnu")) %>%
              #select(datim_uid, usaid_supported),
...
df_cxca <-  df %>% 
  ...
  filter(funding_agency == "CDC",
         ...
         standardizeddisaggregate == "Total Numerator",
         fiscal_year %in% c(2022:metadata$curr_fy), #update years
         psnuuid %in% clinical_cdc_psnu
  ) %>% 
  group_by(fiscal_year, funding_agency, indicator, snu1, psnu, #psnuuid
           cop22_psnu, cop22_psnuuid) %>% 
...
df_cxca %>% 
  filter(indicator == "CXCA_SCRN",
         #period %in% c("FY23Q4", metadata$curr_pd)) %>% #update periods
         period >= "FY22Q4") %>% 
  ggplot() +
  geom_point(mapping = aes(x = period, y = value, 
                           color = usaid_supported),
...
df_cxca_ag <- df %>% 
  ...
  filter(funding_agency == "CDC",
         str_detect(indicator, "CXCA"),
         standardizeddisaggregate == "Total Numerator",
         #fiscal_year %in% c(metadata$curr_fy - 1, metadata$curr_fy),
         fiscal_year >= metadata$curr_fy - 2,
         psnuuid %in% clinical_cdc_psnu
  ) %>% 
  group_by(fiscal_year, funding_agency, indicator, snu1,# psnu, psnuuid
           cop22_psnu, cop22_psnuuid) %>% 
  ...
  filter(period %ni% c(max(period),min(period)))
...
a <- df_cxca_ag %>% 
  ...
  si_style_xline() +
  facet_wrap(~usaid_supported) +
  geom_hline(yintercept = seq(1e4, 3.5e4, 1e4), color = "white", linewidth = 0.5) +
  ...
  labs(x = NULL, y = NULL,
       title = glue("USAID-SUPPORTED WOREDAS HAD SLIGHTLY HIGHER NUMBERS OF
                    ...
                    <span style = 'color:{si_blue}'>TREATMENT LINKAGE</span>"))
...
b <- df_cxca_ag %>% 
  ...
  #         family = "Source Sans Pro",
  #        size = 12/.pt) +
  geom_hline(yintercept = 1, linewidth = 0.25, linetype = "dotted") +
...
df_hts <- df %>% 
  ...
  filter(funding_agency == "CDC",
         ...
         standardizeddisaggregate == "Total Numerator",
         fiscal_year %in% c(metadata$curr_fy -1, metadata$curr_fy), #update FY
         psnuuid %in% clinical_cdc_psnu) %>% 
  group_by(fiscal_year, funding_agency, indicator, snu1, psnu, #psnuuid
           cop22_psnu, cop22_psnuuid) %>% 
  summarise(across(starts_with("qtr"), sum, na.rm = TRUE),.groups = "drop") %>%
...
df_rtt <-  df %>% 
  ...
  filter(funding_agency == "CDC",
         ...
         standardizeddisaggregate == "Total Numerator",
         fiscal_year %in% c(metadata$curr_fy -1, metadata$curr_fy), #update years 
         psnuuid %in% clinical_cdc_psnu
  ) %>% 
  group_by(fiscal_year, funding_agency, indicator, snu1, psnu, #psnuuid
           cop22_psnu, cop22_psnuuid) %>% 
...
df_rtt_agg %>% 
  ...
  labs(x = NULL, y = NULL, title = glue("<span style = 'color:#855C75'>USAID-SUPPORTED</span> WOREDAS ACCOUNT FOR
   OVER HALF OF THE TOTAL PATIENTS RETURNING TO TREATMENT"),
...
si_save("Graphics/04a_rtt_bar.svg")
...
df_rtt_agg_2 %>% 
  filter(snu1 %in% c("Addis Ababa", "Oromia", "Amhara"),
         period %in% c("FY23Q2","FY23Q3","FY23Q4",metadata$curr_pd)) %>% #4 quarters
  ggplot(aes(x = period)) +
  ...
  scale_y_continuous(limits = c(0, 1800), #update
                     label = label_number(scale_cut = cut_short_scale()))+
  si_style_ygrid() +
  theme(legend.position = "none",
        plot.title = ggtext::element_markdown())+
...
df_txnew <-  df %>% 
  ...
  filter(funding_agency == "CDC",
         ...
         standardizeddisaggregate %in% c("Age/Sex/HIVStatus",
                                         ...
                                         "Age/Sex/CD4/HIVStatus"), #cd4 disaggs
         fiscal_year %in% c(metadata$curr_fy-1, metadata$curr_fy), #update years of interest
         psnuuid %in% clinical_cdc_psnu
  ) %>% 
  group_by(fiscal_year, funding_agency, indicator, snu1, psnu, #psnuuid
           cop22_psnu, cop22_psnuuid) %>% 
...
df_txnew_agg %>% 
  ...
  geom_text(data = . %>% filter(period == metadata$curr_pd),
            aes(y = TX_NEW,
                label = comma(TX_NEW)), #clean_number
            size = 11/.pt, 
            family = "Source Sans Pro", 
            color = grey90k,
            vjust = -0.95
  ...
  geom_text(data = . %>% filter(period == metadata$curr_pd),
            aes(y = TX_NET_NEW,
                label = comma(TX_NET_NEW)), #clean_number
            size = 11/.pt, 
            family = "Source Sans Pro", 
            color = grey90k,
            vjust = -0.95
  ...
  scale_y_continuous(#limits = c(0,6000), #update
                     label = label_number(scale_cut = cut_short_scale())) +
  theme(plot.title = ggtext::element_markdown())+
...
df_tb <-  df %>% 
  ...
  gophr::clean_indicator() %>% #applies denominator suffix
  group_by(fiscal_year, funding_agency, indicator, snu1, psnu, #psnuuid
           cop22_psnu, cop22_psnuuid) %>% 
  summarise(across(starts_with("qtr"), sum, na.rm = TRUE),.groups = "drop") %>%
  reshape_msd() %>% 
...
df_tb_agg %>%
  filter(period %in% c("FY22Q4", "FY23Q2","FY23Q4", metadata$curr_pd)) %>% #update
  ggplot(aes(x = period)) +
